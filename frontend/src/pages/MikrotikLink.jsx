...
export default function MikrotikSettings() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setLastTest({ ok: true, mode: data.mode, message: data.message, profileCount: data.profile_count, warning: data.warning });
  ...
  const pullRouterStatus = async () => {
    if (!form.mikrotik_host.trim() || !form.mikrotik_user.trim() || (!form.mikrotik_pass.trim() && !hasSavedPassword)) {
      toast.error('Enter or save MikroTik host, username, and password first');
      return;
    }
    setPullingStatus(true);
    try {
      const { data } = await api.post('/router/status', payload());
      setRouterStatus(data);
      toast.success(data.source === 'provisioning_snapshot' || data.source === 'provisioning_seen' ? 'Loaded router provisioning state' : 'Router status pulled');
    } catch (error) {
    ...
    <div className="space-y-4">
      <section id="link-mikrotik" className="surface-card p-4">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="page-subtitle">Connect the router, load pushed configuration, and assign ports for PPPoE or Hotspot service.</p>
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Run this once in the MikroTik terminal to enable API access and prepare captive portal defaults. Your saved password below should be the router login password.</p>
          </div>
        ...
        {provision && (
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
              ...
              <p className="mt-1 text-sm font-semibold text-slate-900">Enable API and captive portal defaults</p>
            </div>
          </div>
        )}
      ...
      <form className="surface-card p-4" onSubmit={save}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label className="form-label" htmlFor="mikrotik_user">Router username</label>
            <input id="mikrotik_user" name="mikrotik_user" className="form-input" value={form.mikrotik_user} onChange={update} placeholder="admin" />
          </div>
          <div>
            <label className="form-label" htmlFor="mikrotik_pass">Router password</label>
      ...
      {lastTest && (
        <section className={`rounded-lg border p-4 ${lastTest.ok ? (lastTest.mode === 'provisioning_callback' ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-green-200 bg-green-50 text-green-800') : 'border-red-200 bg-red-50 text-red-700'}`}>
          <p className="text-sm font-medium">{lastTest.message}</p>
          {lastTest.warning && <p className="mt-1 text-sm">{lastTest.warning}</p>}
      ...
      <section className="surface-card p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Load device details, profiles, active services, and physical interfaces pushed during provisioning.</p>
          </div>
          <button type="button" className="btn-primary" onClick={pullRouterStatus} disabled={pullingStatus}>
            <Network size={17} />
